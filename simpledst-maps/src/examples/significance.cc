...
//    SkyMap Emap0; 
//    read_Healpix_map_from_fits(input[3], Emap0,     1);
// 
//    std::vector<double> norm;
//    std::ifstream filein(input[4].c_str());
//    for (std::string line; std::getline(filein, line); )
//    {
//            norm.push_back( std::atof( line.c_str() ) );
//    }
//    std::vector<double> norm0;
//    std::ifstream filein2(input[5].c_str());
//    for (std::string line; std::getline(filein2, line); )
//    {
//            norm0.push_back( std::atof( line.c_str() ) );
//    }
    SkyMap bkg;
    ...
    for (unsigned int i=0; i < npix;i++ )
    {
            ...
            diffCRmapSmooth[i] = sumDat/sumBkg - 1.;
             // WRONG
            significance[i] = sumDat / sqrt(sumBkg);
