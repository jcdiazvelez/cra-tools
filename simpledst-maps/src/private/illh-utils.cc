...
/*
 * write_maps - save results to FITS file
 */
void illh::write_maps(
                ...
                unsigned iteration)
{
    ...
    namefits << foldername << boost::format("/%s_%s_%d_%d_iteration%02d.fits.gz") % prefix % detector  % nsideOut % nTimesteps % iteration;
    ...
    fitsOut.add_comment("Maps: dI, data, bg");
    prepare_Healpix_fitsmap(fitsOut, CRMap, PLANCK_FLOAT64, colname);

    fitsOut.write_column(1, CRMap.Map());
    fitsOut.write_column(2, dataMap.Map());
    fitsOut.write_column(3, bkgMap.Map());
    fitsOut.close();
}
