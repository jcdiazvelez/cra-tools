...
// 11/2015
// =========================================================================
//  File Name     : TimeScramble.cpp
//  Description   : Performs time-scrambling algorithm on IceCube/IceTop
// 				    data. Creates data, reference, and local coordinate maps.
//                  performance related
//  Creation Date : The beginning of time
...
void TimeScramble(po::variables_map variables_map, string inFile) {
	...
	I3Time time;
	time.SetUTCCalDate(yyyy, mm, dd, 0, 0, 0);
	double mjd_input_file = time.GetModJulianDayDouble();

	//time.SetTime(mjd_input_file - 1);	// Previous day
	I3Time prev_day(mjd_input_file - 1);	// Previous day
	string cal_string_prev = prev_day.GetCalendarString();
	string yyyymmdd_prev = cal_string_prev.substr(0, 10);
	...
	I3Time post_day(mjd_input_file + 1);	// Previous day
	string cal_string_post = post_day.GetCalendarString();
	string yyyymmdd_post = cal_string_post.substr(0, 10);
...
//	}
	...
	Long64_t nEntries = cutDST->GetEntries();	// Sum of number of entries for each TTree in the TChain
	...
	for (Long64_t jentry = 0; jentry < nEntries; ++jentry) {
		...
		if (pass_cuts && dst.ModJulDay <= (startMJD+dt)) {
			...
			time.SetModJulianTimeDouble(dst.ModJulDay);
			// Start here *********
			//local.SetLocalRad(zenith, azimuth);
			local.SetThetaPhi(zenith, azimuth);
