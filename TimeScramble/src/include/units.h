...
/**
 ...
 I just wrapped it all in a namespace Units, Also changed the
 ...
 *
 * The Units namespace contains a bunch of "static const double's" which
 * define the units that are used in the dataclasses.  The header originally
 * comes from Geant4, and wrapping it in and Units namespace is an IceCube
 * modification.
 * 
 ...
 *    For example:
 *      double myvoltage = 45.00*units::V;
 *      myContainerPtr->SetVoltage(myvoltage);
 *
 *      .....
 *      readvoltage = myContainerPtr->GetVoltage()/units::V;
 *
 * @version $Version:$
 * @date $Date$
 ...
 * <tt> using namespace constants </tt> to get access to them
 * directly, or just use @c constants::pi, for example.
 * 
...
namespace constants
{
   ...
   */
  static const double c = 2.99792458e8 * units::m / (units::second);

  /**
   * Impedance of free space
   */

  static const double z_vacuum = 376.730313668 * units::ohm;
   ...
   */
  static const double theta_cherenkov = std::acos(1/n_ice_phase) * units::rad;
   ...
   */
  static const double SurfaceElev = 9291.31*units::feet;
   ...
   * @sa http://wiki.icecube.wisc.edu/index.php/Coordinate_system
   */
  static const double OriginElev = 2900*units::feet;
   ...
   */
  static const double earthRadius = 6.371315e6*units::m;
   ...
   */
  static const double Coordinate_shift_x = -339.8*units::m;
  static const double Coordinate_shift_y = -117.4*units::m;
  static const double Coordinate_shift_z = -216.0*units::m;
   ...
   */
  static const double dt_window_l = -15*units::ns;
  static const double dt_window_h = +25*units::ns;
};
