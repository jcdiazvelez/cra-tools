...
int main(int argc, char* argv[]) {
  ...
  string arr[] = {"outBase", "config", "nInt", "method", "yyyymmdd", "sd"};
  int nKeys = 6;
  ...
  //vector<string> test = vm["inFiles"].as< vector<string> >();
  ...
  //for (unsigned i = 0; i < fileList.size(); ++i) {
  //  const char* inFiles = fileList[i].c_str();
  //  cout << "Input files are: " << inFiles << endl;
...
//void tScramble(po::variables_map vm) {
//void tScramble(po::variables_map vm, const char* inFilesStr_) {
void tScramble(po::variables_map vm, vector<string> inFiles_) {
  ...
  //vector<string> inFiles_ = vm["inFiles"].as<vector <string> >();
  // Split infiles by space
  //string inFilesStr = inFilesStr_;
  //stringstream ss(inFilesStr);
  //istream_iterator<string> begin(ss);
  //istream_iterator<string> end;
  //vector<string> inFiles_(begin, end);
  ...
  for (Long64_t jentry=0; jentry<nEntries; ++jentry) {
    ...
    if (isGood && dst.ModJulDay <= (startMJD+dt)) {
      ...
      if (sd) {
        eventweight = solar_dipole(dst.ModJulDay, eq.ra, eq.dec);
      }
